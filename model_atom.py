...
from torch_geometric.nn import NNConv, Set2Set, GCNConv, GINEConv
...
class MolAC_GCN(nn.Module):
    def __init__(self, nhid, nvec, num_out_per_stem, num_out_per_mol, num_conv_steps, version, dropout_rate=0, proxy_num_dropout_samples=0, do_stem_mask=True, do_nblocks=False):
    ...
    def action_negloglikelihood(self, s, a, g, stem_o, mol_o):
        ...
        stem_slices = torch.tensor(
            s.__slices__['stems'][:-1], dtype=torch.long, device=stem_lsm.device)
        return -(
    ...
    def index_output_by_action(self, s, stem_o, mol_o, a):
        stem_slices = torch.tensor(
            s.__slices__['stems'][:-1], dtype=torch.long, device=stem_o.device)
        return (
    ...
    def posterior(self, x):
        ...
        posterior = posterior(outputs.mean(dim=0), outputs.var(dim=0))
