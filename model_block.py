...
from torch_geometric.nn import GINConv, GINEConv, GCNConv
from torch_geometric.nn import global_mean_pool
from torch_sparse import SparseTensor


class GraphAgent(nn.Module):
    def __init__(self, nemb, nvec, out_per_stem, out_per_mol, num_conv_steps, mdp_cfg, version='v4', partition_init=0):
        super().__init__()
        print(version)
        ...
        self.block2emb = nn.Sequential(nn.Linear(nemb + nvec_1, nemb), nn.LeakyReLU(), # nvec=0, if version=='v4'
                                       nn.Linear(nemb, nemb))
    ...
    def forward(self, graph_data, vec_data=None, do_stems=True):
        ...
        mol_preds = self.global2pred(
            gnn.global_mean_pool(out, graph_data.batch))
...
class GraphAgent_FiLM(GraphAgent):
    def __init__(self, nemb, nvec, out_per_stem, out_per_mol, num_conv_steps, mdp_cfg, version='v4', partition_init=0):
        super().__init__(nemb, nvec, out_per_stem, out_per_mol,
                         num_conv_steps, mdp_cfg, version, partition_init)
        self.cond2emb = nn.Sequential(nn.Linear(nvec, nemb), nn.LeakyReLU(),
                                      nn.Linear(nemb, 2 * nemb * num_conv_steps))

    def forward(self, graph_data, vec_data, do_stems=True):
        blockemb, stememb, bondemb = self.embeddings
        graph_data.x = blockemb(graph_data.x)
        if do_stems:
            graph_data.stemtypes = stememb(graph_data.stemtypes)
        ...
        if self.version == 'v1' or self.version == 'v3':
            ...
            out = self.block2emb(out)
            condition = self.cond2emb(
                vec_data).view(-1, self.num_conv_steps, self.nemb, 2)
            condition = condition[graph_data.batch]

        h = out.unsqueeze(0)

        for i in range(self.num_conv_steps):
            m = self.conv(out, graph_data.edge_index, graph_data.edge_attr)
            m = condition[:, i, :, 0]*out + condition[:, i, :, 1]
            m = F.leaky_relu(m)
            out, h = self.gru(m.unsqueeze(0).contiguous(), h.contiguous())
            out = out.squeeze(0)
            ...
            # out, h = self.gru(m.unsqueeze(0).contiguous(), h.contiguous())
            # out = out.squeeze(0)
            # out = condition[:, i, :, 0]*out + condition[:, i, :, 1]
        ...
        if do_stems:
            stem_block_batch_idx = (
                ...
                + graph_data.stems[:, 0])
            if self.version == 'v1' or self.version == 'v4':
                # TODO  input = node_repr || stem_repr, maybe global_repr is useful
                stem_out_cat = torch.cat(
                    [out[stem_block_batch_idx], graph_data.stemtypes], 1)
            elif self.version == 'v2' or self.version == 'v3':
                stem_out_cat = torch.cat([out[stem_block_batch_idx],
...
class GraphAgent_Concat(GraphAgent):
    def __init__(self, nemb, nvec, out_per_stem, out_per_mol, num_conv_steps, mdp_cfg, version='v1', partition_init=150.0):
        super().__init__(nemb, nvec, out_per_stem, out_per_mol,
                         num_conv_steps, mdp_cfg, version, partition_init)

        nvec_2 = nvec * (version == 'v2' or version == 'v3')
        self.stem2pred = nn.Sequential(nn.Linear(nemb * 3 + nvec_2, nemb), nn.LeakyReLU(),
                                       nn.Linear(nemb, nemb), nn.LeakyReLU(),
                                       nn.Linear(nemb, out_per_stem))
        self.global2pred = nn.Sequential(nn.Linear(nemb * 2, nemb), nn.LeakyReLU(),
                                         nn.Linear(nemb, out_per_mol))
        
        self.ray_mlp = torch.nn.Sequential(
            torch.nn.Linear(nvec, nemb),
        ...
        self.linear = nn.Linear(nemb*2, nemb)
    ...
    def forward(self, graph_data, vec_data=None, do_stems=True):
        ...
        graph_data.edge_attr = (
            graph_data.edge_attr[:, 0][:, :, None] *
            graph_data.edge_attr[:, 1][:, None, :]
        ).reshape((graph_data.edge_index.shape[1], self.nemb**2))  # out-product and flatten
        out = graph_data.x
        if self.version == 'v1' or self.version == 'v3':
            batch_vec = vec_data[graph_data.batch]
            out = self.block2emb(torch.cat([out, batch_vec], 1))
            ...
            if vec_data is not None:
                condition = self.ray_mlp(vec_data) 

        h = out.unsqueeze(0)
        # print(out.shape, condition.shape)
        out = self.linear(torch.cat([out, condition[graph_data.batch]],1))
        ...
        if do_stems:
            ...
            if self.version == 'v1' or self.version == 'v4':
                # TODO  input = node_repr || stem_repr, maybe global_repr is useful
                stem_out_cat = torch.cat(
                    [out[stem_block_batch_idx], graph_data.stemtypes, condition[graph_data.stems_batch]], 1)
            elif self.version == 'v2' or self.version == 'v3':
                stem_out_cat = torch.cat([out[stem_block_batch_idx],
                                          graph_data.stemtypes,
                                          vec_data[graph_data.stems_batch]], 1)
        ...
        # mol_preds = self.global2pred(
        #     gnn.global_mean_pool(torch.cat([out, condition[graph_data.batch]],1), graph_data.batch))

        mol_preds = self.global2pred(
            torch.cat([gnn.global_mean_pool(out, graph_data.batch), condition], 1))

        return stem_preds, mol_preds


def mol2graph(mol, mdp, floatX=torch.float, bonds=False, nblocks=False):
    def f(x): return torch.tensor(x, dtype=torch.long, device=mdp.device)
    ...
    stemtypes = [mdp.stem_type_offset[t[mol.blockidxs[i[0]]]] + i[1]
                 for i in mol.stems]

    data = Data(x=f([t[i] for i in mol.blockidxs]),
                edge_index=f(edges).T if len(edges) else f([[], []]),
                edge_attr=f(edge_attrs) if len(
                    edges) else f([]).reshape((0, 2)),
