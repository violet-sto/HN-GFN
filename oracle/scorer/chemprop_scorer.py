...
class chemprop_model():
    ...
    def __call__(self, smiles, batch_size=200):
        test_data = get_data_from_smiles(smiles=smiles, skip_invalid_smiles=False, features_generator=['rdkit_2d_normalized'])  #! 300 dim + 200 dim
...
def get_scores(task, mols, device=None):
    ...
    if model is None:
        ...
        elif task == 'chemprop_hiv':
            model = chemprop_model(os.path.join(ROOT_DIR, 'chemprop_ckpt/hiv'), device=device)
        else: raise NotImplementedError
        models[task] = model

    if len(mols) > 0:
        smiles = [[Chem.MolToSmiles(mol)] for mol in mols]  #! smiles --> smiles list
